...
The Food Machine AI is disabled by default. Set `GEMINI_API_KEY`, optionally set `GEMINI_MODEL` and `AI_COOLDOWN_MS` in `.env`, and then enable it in a server with `/ai enable`. `GEMINI_MODEL` defaults to `gemini-3.6-flash`, and `AI_COOLDOWN_MS` defaults to `15000`. AI requests send the gathered Discord context to the Google Gemini API.
